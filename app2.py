...
with st.form("cancel_form"):
    cancel_email = st.text_input("Enter your email")
    cancel_date = st.date_input("Enter your travel date")
...
if cancel_button:
    if cancel_email:
        ...
        # Find booking
        c.execute('''SELECT bookings.id, buses.route, bookings.travel_date, bookings.seat_number, bookings.bus_id 
                     FROM bookings
                     JOIN customers ON bookings.customer_id = customers.id
                     JOIN buses ON bookings.bus_id = buses.id
                     WHERE customers.email=? AND bookings.travel_date=?''', 
                  (cancel_email, str(cancel_date)))
        
        booking = c.fetchone()
        
        if booking:
            booking_id, route, travel_date, seat_number, bus_id = booking
            
            # Check time difference
            travel_datetime = datetime.strptime(str(travel_date), "%Y-%m-%d")
            ...
            if time_diff.total_seconds() >= 12 * 3600:  # 12 hours
                ...
                st.success("Booking canceled successfully! Email sent.")
                
                email_body = f"""
Dear Customer,

Your booking for {route} on {travel_date} has been successfully canceled.

Thank you,
IMMANUEL Travels
"""
                send_email(cancel_email, "Booking Cancellation - IMMANUEL Travels", email_body)
            else:
                st.error("Cannot cancel. Cancellations must be at least 12 hours before travel.")
        else:
            st.error("No booking found for this email and date.")
